...
class DeckProbability:
    ...
    def score(draw_tree : DrawTree, Cs: List[int], mulligan_threshold = 0.0):
        for _ in DeckProbability.recursive_tree_probability(draw_tree, Cs, 1.0):
            pass
        return DeckProbability.score_with_mulligan(draw_tree, mulligan_threshold)
    
    @staticmethod
    def recursive_tree_probability(draw_tree : DrawTree, Cs: List[int], probability : float):
        ...
        if turn_probability > 0 and draw_tree.best_child_impact > 0:
            if len(draw_tree.children) == 0:
                # leaf node, final turn
                draw_tree.expected_impact = draw_tree.best_child_impact
                yield turn_probability, [0,0,0,0,0]
            else:
                for child in draw_tree.children:
                    yield from DeckProbability.recursive_tree_probability(child, new_Cs, turn_probability)
